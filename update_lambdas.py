...
def create_zip_file(zip_path: str, file_path: str, uploaded_file: str):
    with zipfile.ZipFile(zip_path, 'w') as zipf:
        zipf.write(file_path, uploaded_file)
    ...
    for root, dirs, files in os.walk(SOURCE_DIR):
        for uploaded_file in files:
            if is_python_script(uploaded_file):
                ...
                create_zip_file(zip_path, file_path, uploaded_file)
                upload_file(zip_path)
                remove_file(zip_path)

                print(f'File: {uploaded_file} has been uploaded successfully')
